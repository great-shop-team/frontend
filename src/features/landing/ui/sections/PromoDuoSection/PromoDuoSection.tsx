      ...
      <div className="mx-auto grid w-full max-w-[1280px] grid-cols-1 gap-5 md:grid-cols-2">
        {promoDuo.map((item) => (
          ...
          >
            ...
            />
            <div className="absolute inset-x-0 bottom-0 z-1 flex flex-col items-start gap-3 p-5 md:gap-4 md:p-8">
              <h3 className="m-0 font-(family-name:--font-unbounded) text-[20px] font-medium text-white md:text-[25px]">
                {item.label}
              </h3>
              <span className="inline-flex h-11 items-center justify-center rounded-[10px] border-2 border-[#fafafa] bg-transparent px-5 font-(family-name:--font-unbounded) text-sm font-normal text-[#fafafa] transition-opacity group-hover:opacity-85 md:px-[38px] md:text-base">
                {labels.shopNow}
              </span>
            </div>
          </Link>
